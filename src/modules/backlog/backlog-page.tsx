...
import { mockBacklog, mockSprints } from '@/modules/shared/data/mockData'
...
export function BacklogPage() {
  ...
  const handleMoveToSprint = (storyId: string, sprintId: string) => {
    ...
    if (storyIndex !== -1) {
      ...
      const fromSprint = sprints.find((sprint) => sprint.userStories.some((s) => s.id === storyId))
  ...
  const handleMoveToBacklog = (storyId: string, fromSprintId?: string) => {
    ...
    if (!sourceSprintId) {
      const sourceSprint = sprints.find((sprint) => sprint.userStories.some((s) => s.id === storyId))
    ...
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className='w-full'>
        ...
        <div className='flex-1 overflow-auto p-6 space-y-6'>
          <div
            ref={setBacklogRef}
            className={`bg-white rounded-lg border p-4 min-h-32 ${isBacklogOver ? 'ring-2 ring-blue-500' : ''}`}
          >
          ...
          <div>
            ...
            <SprintList sprints={sprints} onMoveToBacklog={handleMoveToBacklog} onMoveToSprint={handleMoveToSprint} />
          </div>
        </div>
      </div>

      <DragOverlay>{activeStory ? <UserStoryCard story={activeStory} isDraggable={false} /> : null}</DragOverlay>
    </DndContext>
  )
}
